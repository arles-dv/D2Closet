...
const d2APIRootPath = 'https://www.bungie.net/Platform'

// All error codes related to the api,
// https://bungie-net.github.io/index.html#/components/schemas/Exceptions.PlatformErrorCodes
enum D2APIErrorCode {
    None = 0,
    Success = 1,
...
// The raw response returned by the destiny API. 
interface D2APIResponse {
    ErrorCode: D2APIErrorCode,
...
interface D2APIRequestInit {
    method: string,
    headers?: HeadersInit,
    body?: any
}

export async function D2APIRequest(endpoint: string, init?: D2APIRequestInit): Promise<D2APIResponse>
{
    const headers = init?.headers ? { ...init.headers, 'X-API-Key': ENV.d2APIKey } : { 'X-API-Key': ENV.d2APIKey }

    const req = await fetch(`${d2APIRootPath}${endpoint}`, {
